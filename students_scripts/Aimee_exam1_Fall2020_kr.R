...
R_elia = load("test1_data_2020/aurelia_15minCell_statareas.Rdata")
  ...
  # correction: -0.25 ponts
  load("test1_data_2020/aurelia_15minCell_statareas.Rdata")
  # Note that the object 'R_elia' does not show up in your environment when line 270 is run; instead the object 'a' becomes available
  # the object name of the 'R' data set is automatically assigned (whichever object it was saved as) when you load it.
  # This is the execption to the rule about assigning data sets to objects when you read the file in.

relia_txt = read.table("test1_data_2020/aurelia_15minCell_statareas.txt", stringsAsFactors = FALSE)

  #correction -0.5 points
  head(relia_txt) # this shows that the data was not read in properly. R needs more information about the delimiter.
  #in this case, it needs to know that the file is comma delimited and that there are headers
  relia_txt = read.table("test1_data_2020/aurelia_15minCell_statareas.txt", stringsAsFactors = FALSE, sep = ",", header = T)
  head(relia_txt)
...
OST = read.csv("test1_data_2020/OST14_1E_d5_frames.csv", stringsAsFactors = FALSE)
  # correction -0.5 points
  # this shows that the data was not read in properly. R needs more information about the delimiter.
  # in this case, it needs to know that the file is comma delimited and that there are headers and it should skip 2 lines
  OST = read.csv("test1_data_2020/OST14_1E_d5_frames.csv", stringsAsFactors = FALSE,sep = ",", header = T, skip = 2)
  head(OST)
...
westcoast = as.POSIXct(strptime(x = t, format = "%Y/%m/%d %H:%M:%OS", tz = "America/Los_Angeles"))
  ...
  #hey there, the code does not work but here is a fix. You were 90% the way there. The problem as in the format argument for the strptime function
  #note that the date format for object 't' goes month/day/year, so the format needs to follow that pattern.
  #you currently have the format as year/month/day. Swich that around and tadah!
  westcoast = as.POSIXct(strptime(x = t, format = "%m/%d/%Y %H:%M:%OS", tz = "America/Los_Angeles"))
