...
# 16) Using indexing, extract the 2nd, 6th, and 9th values from the 3rd column of the matrix your created in Question 14.
    MAT[c(2,6,9),3] #clever use of syntax; +0.25 points
...
is.factor(d$tow)
  ...
  #correction: -0.25 points
    # your are using the correct function to convert from a character to a factor here,
    # but you are missing 'd$tow <- ' to indicate you wish the structure to be overwritten in the data frame,
    # otherwise 'R' performs the function but does not make an actual change to the data structure
...
is.integer
  ...
  #correction: -0.25 points
    # your are using the correct function to convert to an interger here,
    # but you are missing 'd$haul <- ' to indicate you wish the structure to be overwritten in the data frame,
    # otherwise 'R' performs the function but does not make an actual change to the data structure
    d$haul <- as.factor(d$haul)
...
#36) Import the following files into 'R' and assign each data set its own unique object.
  ...
  library(ncdf4) #this library is not needed as there are not netCDF files
  library(readxl)
  ...
  A15T = read.table("C:/Users/Blaine/Downloads/test1_data_2020/aurelia_15minCell_statareas.txt",
  ...
  #I looked at this file outside of R using both Excel and Notepad, but I couldn't decide how to seperate the data to make it intelligible.
    ...
    # the first 2 lines are not part of the data set so they need to be skipped
    OST14 = read.csv("C:/Users/Blaine/Downloads/test1_data_2020/OST14_1E_d5_frames.csv", header = F, skip = 2)
