...
# correction: you are correct that you are not allowed to use the above character strings as object names
# however, I was looking for the non-allowed operators (e.g., %,*,+,-,!); -0.75 points
...
#10) create 2 objects, assign one with 4 values and the other with 3 values
  ...
  #correction: yes, that is the error message. Please provide an explanation for the meaning behind the error message
  #-1 point
...
#14) create a die with six sides and sample it with and without replacement. what is the difference between the two methods?
  #correction: -1 point
  die <- sample(x = 1:6, size = 1,replace = TRUE)
  die <- sample(x = 1:6, size = 1,replace = FALSE)
  #the difference is that when 'replace = T' then the value that was drawn
